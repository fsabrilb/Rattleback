...
void Integrator(Body *Spring,Collider Hooke,double dt)
{
  ...
  for(i=0;i<q;i++){Spring[i].Move_R(dt,Xi);} 
  Hooke.AllForces(Spring,dt);
  for(i=0;i<q;i++){Spring[i].Move_V(dt,(1-2*Lambda)/2);}
  for(i=0;i<q;i++){Spring[i].Move_R(dt,Chi);} 
  Hooke.AllForces(Spring,dt);
  for(i=0;i<q;i++){Spring[i].Move_V(dt,Lambda);} 
  for(i=0;i<q;i++){Spring[i].Move_R(dt,1-2*(Chi+Xi));} 
  Hooke.AllForces(Spring,dt);
  for(i=0;i<q;i++){Spring[i].Move_V(dt,Lambda);}
  for(i=0;i<q;i++){Spring[i].Move_R(dt,Chi);} 
  Hooke.AllForces(Spring,dt); 
