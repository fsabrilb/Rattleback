...
double GammaSpring=1e-1/dt;                              /*Coefficients of friction for the spring and the contact force*/
/*------------------------------------------------ADIMENSIONAL VARIABLES-------------------------------------------------------*/
const double MU=0.4;              /*Coefficient for contact force*/
const int Nx=14,Ny=14,Nz=1;       /*Number of springs in each axis*/
...
public:
  ...
  double NormV(void){return norma(V);};
  double NormF(void){return norma(F);};
  double NormFA(void){
    double Fx,Fy,Fz;
    Fx=(q0*q0+q1*q1-q2*q2-q3*q3)*F.x()+2*(q1*q2+q0*q3)*F.y()          -2*(q1*q3-q0*q2)*F.z();
    Fy=2*(q1*q2-q0*q3)*F.x()          +(q0*q0-q1*q1+q2*q2-q3*q3)*F.y()+2*(q2*q3+q0*q1)*F.z();
    Nz=2*(q1*q3+q0*q2)*F.x()          +2*(q2*q3-q0*q1)*F.y()          +(q0*q0-q1*q1-q2*q2+q3*q3)*F.z();
    return sqrt(Fx*Fx+Fy*Fy+Fz*Fz);
  };
  double NormTau(void){return norma(Tau);};
  double NormN(void){return sqrt(Nx*Nx+Ny*Ny+Nz*Nz);};
...
void Body::Draw3D(void){cout<<", "<<X()<<"+"<<r<<"*sin(v)*cos(u),"<<Y()<<"+"<<r<<"*sin(v)*sin(u),"<<Z()<<"+"<<r<<"*cos(v)";}
...
void Body::CalculateTorsion(void){
  ...
  double Tx,Ty,Tz; vector3D T; T=F^R;
  Tx=T.x();Ty=T.y();Tz=T.z();
  Nx=(q0*q0+q1*q1-q2*q2-q3*q3)*Tx +2*(q1*q2+q0*q3)*Ty           -2*(q1*q3-q0*q2)*Tz;
  Ny=2*(q1*q2-q0*q3)*Tx           +(q0*q0-q1*q1+q2*q2-q3*q3)*Ty +2*(q2*q3+q0*q1)*Tz;
  Nz=2*(q1*q3+q0*q2)*Tx           +2*(q2*q3-q0*q1)*Ty           +(q0*q0-q1*q1-q2*q2+q3*q3)*Tz;
}
/*------------------------------------------------CLASS COLLIDER--------------------------------------------------------------*/
class Collider{
private:
...
void StartAnimation3D(double x0Break, double y0Break, double z0Break, double h){
  ...
  cout<<"set palette rgbformulae 21,22,23"<<endl;
}
...
int main(void)
{
  ...
  double Wx1=0.4/dt,Wy1=0,Wz1=0.4/dt,I1=(2.0/5)*m1*r1*r1,I2=I1,I3=I1;/*Initial Conditions Sphere*/
  ...
  Hooke.Start();
  /*Start Animation*/
  //StartAnimation3D(x0Break,y0Break,z0Break,2.5*h);  Ndrawings=1000;
  /*Integration*/
  for(t=tdrawings=0;t<tmax;t+=dt,tdrawings+=dt)
	...
	tdrawings=0;}
      */
      ...
      cout<<t<<"\t"<<Spring[N3D].NormV()<<"\t"<<Spring[N3D].Getr()<<"\t"<<Spring[N3D].NormF()<<"\t"<<Spring[N3D].NormFA()<<"\t"
	  <<Spring[N3D].NormN()<<endl;
    }
  return 0;
}
