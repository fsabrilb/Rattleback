...
void Collider::AllForces(Body* Spring,double dt){
  ...
  for(i=0;i<Nx;i++){for(j=0;j<Ny;j++){
      Spring[0+j*Ny].AddForce_x(-Kx*Spring[0+j*Ny].R.x()); Spring[Nx-1+j*Ny].AddForce_x(-Kx*Spring[Nx-1+j*Ny].R.x());}}
...
int main(void)
{
  ...
  double m1=30,r1=80,Rx1=Nx*x0Break/2,Ry1=(Ny+1)*y0Break,Rz1=0,Vx1=20,Vy1=0 ,Vz1=0,Theta1=0,W1=4; /*Initial Conditions*/
  ...
  for(t=tdrawings=0;t<tmax;t+=dt,tdrawings+=dt)
	...
	tdrawings=0;}
      /*
      for(i=0;i<N2D;i++){cout<<t<<"\t"<<i<<"\t"<<Spring[i].X()<<"\t"<<Spring[i].Y()<<endl;}
      */
