...
const int Nx=10,Ny=10,Nz=1;                /*Number of springs in each axis*/
...
public:
  ...
  void InteractionForce2(Body & Spring1, Body & Spring2,vector3D & l,bool & InCollision,double dt,int i,int j);
};
...
void Collider::AllForces(Body* Spring,double dt){
  ...
  /*Sphere and Spring*/
  for(i=0;i<Nx;i++){for(j=0;j<Ny;j++){InteractionForce2(Spring[j+i*Nx],Spring[N2D],l[i][N2D],InCollision[i][N2D],dt,i,j);}}
}
...
void Collider::InteractionForce2(Body & Spring1, Body & Spring2,vector3D & l,bool & InCollision,double dt,int i,int j){
  ...
  double ERFF=1e-8;
  R1x=Spring1.Rx+i*Spring1.xBreak; R2x=Spring2.Rx; V1x=Spring1.Vx; V2x=Spring2.Vx;
  R1y=Spring1.Ry+j*Spring1.yBreak; R2y=Spring2.Ry; V1y=Spring1.Vy; V2y=Spring2.Vy;
...
int main(void)
{
  ...
  double m1=20,r1=10,Rx1=Nx*x0Break/2,Ry1=Ny*y0Break,Rz1=0,Vx1=0,Vy1=0 ,Vz1=0,Theta1=0,W1=4; /*Initial Conditions*/
  double omegax=sqrt(Kx/m0),omegay=sqrt(Ky/m0),omegaz=sqrt(Kz/m0);                   /*One of Frequencies in each axis*/
  double Tx=2*M_PI/omegax, tmax=3*Tx;                                                /*Time Step and Tmax*/
